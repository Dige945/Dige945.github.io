...
即上述函数可以化�?$$ f(x)= \frac{1}{e^{wx+b}+1} $$
$$ f(x) = max(0,wx+b) $$

tip：RELU用得比较多，因为计算量少，理论上一�?e^x$的计算，就相当于百多次乘法运算，重复使用对算力要求比较高�?
...
#Flatten函数将图像格式从二维数组转换为一维数组，输入形状�?批量大小，通道，高，宽)
def init_weights(m):
    if type(m) == nn.Linear:
...
#分类有十个类
num_outputs = 10
#W为权重，b为偏�?W = torch.normal(0, 0.01, size=(num_inputs, num_outputs), requires_grad=True)
b = torch.zeros(num_outputs, requires_grad=True)

def softmax(X):
    #0是行�?是列，sum里面是哪个就把哪一个压�?    X_exp = torch.exp(X)
    partition = X_exp.sum(1, keepdim=True)
    return X_exp / partition  # 这里应用了广�?
def net(X):
...
def train_epoch_ch3(net, train_iter, loss, updater):
    ...
    for X, y in train_iter:
        # 计算梯度并更新参�?        y_hat = net(X)
        l = loss(y_hat, y)
        if isinstance(updater, torch.optim.Optimizer):
            # 使用PyTorch内置的优化器和损失函�?            updater.zero_grad()
            l.mean().backward()
            updater.step()
        else:
            # 使用定制的优化器和损失函�?            l.sum().backward()
            updater(X.shape[0])
            metric.add(float(l.sum()), accuracy(y_hat, y), y.numel())
    # 返回训练损失和训练精�?    return metric[0] / metric[2], metric[1] / metric[2]
